...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    locale,
    title: tBlog("page_title"),
    description: tBlog("list_meta_description"),
    path: "/blog",
  })
}
  ...
  return (
    <main className="min-h-screen bg-background" style={{ fontFamily: "var(--font-jakarta, system-ui)" }}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-28 pb-16">
        <h1 className="text-3xl sm:text-4xl font-bold text-foreground mb-2">{tBlog("page_title")}</h1>
        <p className="text-muted-foreground mb-10">{tBlog("page_description")}</p>

        {posts.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-border rounded-2xl">
            <p className="text-muted-foreground mb-6">{tBlog("no_posts")}</p>
