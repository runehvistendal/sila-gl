...
import { Anchor, ChevronDown, Calendar, Users, ArrowRight, User } from "lucide-react"
...
export interface TransportOfferBookingRowData {
  id: string
  status: string
  price_ore: number
  created_at: string
...
export default function TransportOfferBookingRow({ booking, alreadyReviewed = false }: Props) {
  ...
  const personName =
    ...
    (booking.isSkipperView ? t("booking_row_role_guest_tr") : t("booking_row_role_skipper"))
  const personId = booking.other_id
  const personRole = booking.isSkipperView ? t("booking_row_role_guest_tr") : t("booking_row_role_skipper")
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 truncate">
            {booking.isSkipperView
              ? `${t("booking_row_role_guest_tr")}: ${personName}`
        ...
        <div className="px-4 sm:px-5 pb-5 border-t border-border pt-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {fmt.dateTime(new Date(booking.desired_date + "T12:00:00.000Z"), {
                day:      "numeric",
                month:    "short",
                year:     "numeric",
              })}
            ...
            <span className="flex items-center gap-1">
              ...
              {booking.num_passengers} {tc("guests")}
            </span>
          </div>
          ...
          <div className="bg-muted/50 rounded-xl p-3 space-y-1.5 text-xs text-muted-foreground">
            <div className="flex justify-between font-semibold text-foreground">
              <span>{tc("total")}</span>
              <span>{formatKr(booking.price_ore)}</span>
            </div>
          </div>

          {booking.status === "accepted" && booking.reviewee_id && (
            alreadyReviewed ? (
